...
const FIELDS: Array<{ key: keyof Settings; label: string; type?: string; hint?: string; options?: string[] }> = [
  ...
  { key: 'toolSteps', label: 'Tool calls', options: ['collapsed', 'expanded'], hint: "collapsed: a turn's tool calls fold into one \"working…\" line (failures stay visible) · expanded: one line per call" },
