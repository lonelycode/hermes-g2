...
test('steps form a tree under the turn and the answer can be anchored to the top', () => {
  ...
  assert.equal(lines[4], '■ Hermes:')
  assert.equal(lines[5], 'line one')
  // Following: tail visible. Anchored: the answer label is the first line on the page.
  assert.ok(feed.page().includes('line two'))
  feed.anchorTo(answer)
  assert.deepEqual(feed.page().split('\n').slice(0, 2), ['■ Hermes:', 'line one'])
  assert.equal(feed.anchored, true)
  // As the answer grows past the page, the anchored view follows its last line.
  feed.append(answer, '\nline three\nline four\nline five\nline six')
  const page = feed.page().split('\n')
  assert.equal(page[page.length - 1], 'line six')
